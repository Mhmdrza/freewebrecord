...
export default function Page() {
  ...
  const [uploadedOverlay, setUploadedOverlay] = useState<string | null>(null)
  ...
  useEffect(() => () => {
    ...
    if (uploadedOverlay) URL.revokeObjectURL(uploadedOverlay)
  }, [uploadedBackground, uploadedOverlay])
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      if (overlayImageRef.current?.complete) {
  ...
  const handleOverlayUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const objectUrl = URL.createObjectURL(file)
    const image = new Image()
    image.onload = () => { overlayImageRef.current = image }
    image.src = objectUrl
    setUploadedOverlay(objectUrl)
    event.target.value = ''
  }
    ...
    <main className={`studio-shell min-h-screen bg-[#f4f3f0] text-[#242321] ${isDark ? 'theme-dark' : 'theme-light'}`}>
      ...
      <section className="mx-auto grid max-w-[1440px] gap-6 px-5 pb-8 sm:px-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-12">
        ...
        <aside className="flex flex-col gap-4 lg:pt-[74px]">
          ...
          <div className="rounded-2xl border border-[#e3e0da] bg-white p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-semibold">Front layer</h2><p className="mt-1 text-xs text-[#918c83]">Overlay a logo, frame, or microphone.</p></div><button onClick={() => overlayFileRef.current?.click()} className="flex size-9 items-center justify-center rounded-xl bg-[#f4f1ed] text-[#716d66] hover:bg-[#ebe7e1]" aria-label="Upload front layer image"><ImagePlus /></button><input ref={overlayFileRef} type="file" accept="image/*" onChange={handleOverlayUpload} className="hidden" /></div>{uploadedOverlay ? <div className="flex items-center gap-3 rounded-xl bg-[#f6f4f1] p-2"><div className="flex size-14 items-center justify-center overflow-hidden rounded-lg bg-white"><img src={uploadedOverlay} alt="Front layer preview" className="size-full object-contain" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">Overlay image added</p><p className="text-xs text-[#969088]">Included in preview and recording.</p></div><button onClick={() => { URL.revokeObjectURL(uploadedOverlay); setUploadedOverlay(null); overlayImageRef.current = null }} className="flex size-8 items-center justify-center rounded-lg text-[#918c83] hover:bg-white" aria-label="Remove front layer"><X /></button></div> : <button onClick={() => overlayFileRef.current?.click()} className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#d8d3cb] px-3 py-4 text-xs text-[#918c83] hover:bg-[#faf9f7]"><Upload /> Add overlay image</button>}</div>
